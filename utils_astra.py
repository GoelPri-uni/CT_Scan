...
def generate_sinogram_astra(phantom, proj_geom):
    """
    ...
    """
    # 创建ASTRA需要的volume数据
    phantom_id = astra.data2d.create('-vol', proj_geom['VolumeGeometry'], phantom)
    
    # 创建sinogram存储空间
    sinogram_id, sinogram = astra.data2d.create('-sino', proj_geom, 0)
    
    # 配置投影算子
    cfg = astra.astra_dict('FP')
    cfg['ProjectionDataId'] = sinogram_id
    cfg['VolumeDataId'] = phantom_id
    cfg['ProjectorId'] = astra.create_projector('line', proj_geom, 
                                               proj_geom['VolumeGeometry'])
